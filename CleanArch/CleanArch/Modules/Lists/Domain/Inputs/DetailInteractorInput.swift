 ...
 * Estos métodos serán los encargados de ejecutar los casos de uso disponibles, siendo cada uno de éstos
 * los que soliciten datos a un servidor, guarden información en base de datos o UserDefaults, etc.
 * Además, se recomienda hacer uso de un BusinessObject (BO) que permita transformar
 * la información necesaria/recabada de los casos de uso y comunicarla de forma controlada
 * entre las capas conectadas del módulo (presenter e interactor), con el fin de no perder la integridad del módulo
 * y no verse influenciado por ningún tipo de objeto de dominio externo.
 *
 * Notas:
